...
Автоматизация процесса распределения скидок, при создании нескольких заказов клиента.

## 3. Цель и назначение

*   **Цель:** Создать внешнюю обработку/расширение, которая позволит в справочнике «Условия предоставления скидок (наценок)» задать условие:
скидка/наценка применяется только если сумма всех проведённых заказов клиента по текущей сделке (реквизит Сделка в документах ЗаказКлиента) больше или равна порогу, заданному пользователем в настройках условия..
*   **Назначение:** Функционал предназначен для менеджеров отдела продаж.

### 4. Настройки обработки
...
В справочнике «Условия предоставления скидок (наценок)» появляется новое условие
При настройке открывается форма с одним полем «Порог суммы»
После сохранения в описании и автонаименовании отображается введённое значение
При расчёте скидок в заказе/реализации/коммерческом предложении условие проверяется корректно

### 4.3. Обработка доступных товаров
*   **3.1** Группируем товары по Доступности на складах из списка складов.
  * **3.2** Отбираем товары, которые доступны на складах. 
  ...
  * **3.8** Дозаполняем поля, если сами не ставятся, менеджер, сделка, комментарии . 
