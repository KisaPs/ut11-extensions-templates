...
*   **1.2:** Должен содержать как минимум следующие реквизиты (значения заполняются вручную):
    ...
    * Список складов для просмотра, обхода в циклах:
        ...
        - `Договор продавца ИФЗ` - убрано
        - `Список складов ИФЗ`
        - `Организация продавец`
        - `Договор продавца` - убрано
        - `Склад продавца`
...
*   **2.2:** При нажатии на кнопку запускается алгоритм разбиения, описанный ниже.
  * **2.3:** Проверка на наличие сделки. Перед созданием новых документов, убедиться, что сделка в окружении КП имеется, если нет, создать её и подставить в создаваемые документы.
...
*   **3.1** Группируем товары по Доступности на складах из списка складов.
  ...
  * **3.5** На каждую группу создаем заказ клиента с реквизитом склада, Организация продавец в ИФЗ, Договор продавца ИФЗ берем первый действующий в статусе Действует и организации покупателя .
  * **3.6** То же самое делаем с остальными товарами не входящие по фильтру производителя ифз, подставляя реквизиты не Ифз. 
